...
def updateRecordService(window_end=None):
    ...
    # url = settings.ZOOM_REPORT_DAILY +
    # ("/?year={year}&month={month}".format(
    #   year=timezone.now().year,
    #   month=timezone.now().month))
    # headers = {'Authorization': settings.ZOOM_TOKEN,
    ...
    json_data = open(os.getenv("ZOOM_REPORT_JSON"))
    ...
    try:
        ...
        usage_records = Zoom.objects.filter(date__gt=window_end - timedelta(days=int(os.getenv("DAYS_OF_USAGE"))),
        ...
        data_list = []
        for data in api_response['dates']:
            record_date = datetime.strptime(data['date'], '%Y-%m-%d').date()
            if window_start < record_date <= window_end:
                if not usage_records.filter(date=record_date):
                    zoom_usage_record = Zoom(date=data['date'], new_users=data['new_users'], meetings=data['meetings'],
                                             participants=data['participants'], meeting_minutes=data['meeting_minutes'], )
                    data_list.append(zoom_usage_record)
        Zoom.objects.bulk_create(data_list)
        print("Records Updated!")
    except Zoom.DoesNotExist:
        print('Zoom Record does not exist!')
